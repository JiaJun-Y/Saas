  ...
  var RENDERAPI = {
    ...
    pickerFooterTpl: function(nameOptions, className, text) {
      var html = '<div class="c-datepicker-picker__footer" style="">' +
        '<button id="datepicker-self-red">红色 ≤ 2 ，</button>' +
        '<button id="datepicker-self-mark">黄色 ≤ 5 </button>' +
  ...
  $.extend(Day.prototype, {
    ...
    eventSingle: function() {
      ...
      function setValue(activeNum, input, inputDay) {
        ...
        this.picker.$input.val(inputVal); // input框中时间
      }
    },
    eventRange: function() {
      // 点击选择日期
    ...
    renderHtml: function(year, month, activeDay) {
      ...
      for (var index = 0; index < day; index++) {
        ...
        if (!isWearPopShow) {
          _.find(orangeData, function(O) {
            if (year == O.slice(0, 4) && API.fillTime(month) == O.slice(5, 7)) {
              if (API.fillTime(_val) == O.slice(8, 10)) {
                className += ' orange';
              }
            }
          })

          _.find(redData, function(R) {
            if (year == R.slice(0, 4) && API.fillTime(month) == R.slice(5, 7)) {
              if (API.fillTime(_val) == R.slice(8, 10)) {
                className += ' red';
              }
            }
          })
  ...
  $.extend(SingleDatePicker.prototype, {
    ...
    event: function() {
      ...
      this.$container.on('click', '.c-datepicker-date-picker__next-btn.month', function(event) {
          event.stopPropagation();
          var _this = API.getPicker($(this));
          // var _currentMonth = _this.datePickerObject.initInputVal.slice(5, 7)
      ...
      this.$container.on('click', '.c-datepicker-picker__link-btn.confirm', function() {
        $('.c-datePicker__input-time').val('')
        $('.c-datePicker__input-day').val('')
        var _this = API.getPicker($(this));
        if (!_this.$input.val()) {
